...
Curve Fitter::log_tau_vs_log_m_concentration(std::vector<double> monomer_concentration, std::vector<double> half_times) {
	/* 
	 * Compute log (time)
...
double Fitter::get_scaling_exponent(Curve res) {
	Linear_regression L ; 
	std::vector<double> resL = L.linear_regression(res.get_x_axis(),res.get_y_axis(),res.get_x_axis().size()) ;
	return resL[0] ; 
}

Curve Fitter::log_tau_vs_log_m_concentration_fit(Curve res) {
	Linear_regression L ; 
    std::vector<double> resL = L.linear_regression(res.get_x_axis(),res.get_y_axis(),res.get_x_axis().size()) ;
   	std::vector<std::vector<double>> log_off_plot = L.get_predicted_values(res.get_x_axis(), resL[0], resL[1] ) ; 

   	Curve c(log_off_plot[0], log_off_plot[1]) ;
   	return c ; 
} 
